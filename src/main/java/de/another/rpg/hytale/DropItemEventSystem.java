...
public class DropItemEventSystem extends EntityEventSystem<EntityStore, DropItemEvent.Drop> {
    ...
    public void handle(int i
            , @NonNullDecl ArchetypeChunk<EntityStore> archetypeChunk,
                       ...
                       @NonNullDecl DropItemEvent.Drop dropItemEvent) {

       // String blockId = dropItemEvent.getBlockType().getId();
        ...
        if (skillData != null) {
            for (UseBlockEffect effect : AnotherRPG.getInstance().getEffectManager().getUseBlockEffects()) {
                //effect.onUse(dropItemEvent , player, skillData, skillManager);
            }
        }
